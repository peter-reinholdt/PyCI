...
void compute_rdm1(const FullCIWfn &wfn, const double *coeffs, double *rdm1){
    auto worker = [&](const FullCIWfn &wfn, const long start, const long end, std::vector<double>*& rdm1_local){
        // prepare working vectors
        long n1 = wfn.nbasis;
        long n2 = wfn.nbasis * wfn.nbasis;
        rdm1_local = new std::vector<double>(2*n2, 0.0);
        double *aa = rdm1_local->data();
        double *bb = aa + wfn.nbasis * wfn.nbasis;
        ...
        for (long idet = start; idet < end; ++idet) {
            ...
            for (long i = 0; i < wfn.nocc_up; ++i) {
                ii = occs_up[i];
                // compute 0-0 terms
                aa[(n1 + 1) * ii] += val1;
                // loop over spin-up virtual indices
                for (long j = 0; j < wfn.nvir_up; ++j) {
                    jj = virs_up[j];
            ...
            for (long i = 0; i < wfn.nocc_dn; ++i) {
                ii = occs_dn[i];
                // compute 0-0 terms
                bb[(n1 + 1) * ii] += val1;
                // loop over spin-down virtual indices
                for (long j = 0; j < wfn.nvir_dn; ++j) {
                    jj = virs_dn[j];
    ...
    if (nthread > wfn.ndet) nthread = wfn.ndet;
    ...
    for (long i = 0; i < nthread; ++i) {
        ...
        v_threads.emplace_back([&, i, start, end](){
                worker(wfn, start, end, results[i]);
            }
        );
    }
    std::fill(rdm1, rdm1 + 2*n2, 0.0);
    for (auto &thread : v_threads) thread.join();
    for (auto *result : results){
        cblas_daxpy(2*n2, 1.0, result->data(), 1, rdm1, 1);
        delete result;
    }
}
...
void compute_transition_rdm1(const FullCIWfn &wfn1, const FullCIWfn &wfn2, const double *coeffs1, const double *coeffs2, double *rdm1) {
    auto worker = [&](const FullCIWfn &wfn1, const FullCIWfn &wfn2, const long start, const long end, std::vector<double>*& rdm1_local){
        long n1 = wfn1.nbasis;
        long n2 = wfn1.nbasis * wfn1.nbasis;
        rdm1_local = new std::vector<double>(2*n2, 0.0);
        double *aa = rdm1_local->data();
        ...
        for (long idet = start; idet < end; ++idet) {
            ...
            val1 = (jdet != -1) ? (coeffs1[idet] * coeffs2[jdet]) : 0.;
            // loop over spin-up occupied indices
            for (long i = 0; i < wfn1.nocc_up; ++i) {
                ii = occs_up[i];
                // compute 0-0 terms
                aa[(n1 + 1) * ii] += val1;
                // loop over spin-up virtual indices
                for (long j = 0; j < wfn1.nvir_up; ++j) {
                    jj = virs_up[j];
                    // 1-0 excitation elements
                    excite_det(ii, jj, det_up);
                    sign_up = phase_single_det(wfn1.nword, ii, jj, rdet_up);
            ...
            // loop over spin-down occupied indices
            for (long i = 0; i < wfn1.nocc_dn; ++i) {
                ii = occs_dn[i];
                // compute 0-0 terms
                bb[(n1 + 1) * ii] += val1;
                // loop over spin-down virtual indices
                for (long j = 0; j < wfn1.nvir_dn; ++j) {
                    jj = virs_dn[j];
                    // 0-1 excitation elements
                    excite_det(ii, jj, det_dn);
    ...
    long n2 = wfn1.nbasis * wfn1.nbasis;
    long nthread = get_num_threads();
    if (nthread > wfn1.ndet) nthread = wfn1.ndet;

    Vector<std::thread> v_threads;
    v_threads.reserve(nthread);
    std::vector<std::vector<double>*> results(nthread, nullptr);
    for (long i = 0; i < nthread; ++i) {
        long start = end_chunk_idx(i, nthread, wfn1.ndet);
        long end = end_chunk_idx(i + 1, nthread, wfn1.ndet);
        end = std::min(end, wfn1.ndet);
        v_threads.emplace_back([&, i, start, end](){
                worker(wfn1, wfn2, start, end, results[i]);
            }
        );
    }
    std::fill(rdm1, rdm1 + 2*n2, 0.0);
